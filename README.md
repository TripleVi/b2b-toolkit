...
[![Version](https://img.shields.io/badge/version-v1.1.0-blue.svg)](https://github.com/TripleVi/b2b-toolkit)
...
![Demo](.github/assets/install.png)
3. **Cấu hình quan trọng cho Extension:**
   * Vào phần quản lý Extension của trình duyệt $
ightarrow$ Chọn **Tampermonkey** $
ightarrow$ **Details**.
...
* **Hover Badge:** Khi di chuột vào các phần tử được highlight (ví dụ: Product Card), một bảng thông tin tóm tắt sẽ xuất hiện phía trên gồm: `Product ID (PID)`, `Variant ID (VID)`, và `Pricing Rule (PL)` áp dụng. Có kèm icon tắt để truy cập thẳng vào trang Shopify Admin của sản phẩm đó.
...
### 3. Custom Selector Feature
Sử dụng khi cần ghi đè (override) hoặc gán selector thủ công cho các thành phần trên giao diện khi tính năng auto installation hoạt động không chính xác do cấu trúc theme.

Toàn bộ công cụ có thể truy cập qua namespace: `BSS_B2B.support.customSelector`.
...
### 4. Custom Function Feature
Hỗ trợ đăng ký và thực thi các đoạn code tùy biến ngay khi công cụ bắt đầu chạy, giúp inject nhanh các đoạn script test hoặc custom handler.
...
* **Start with function keyword:** Bắt đầu bằng từ khóa `function` bắt buộc.
* **Give a name of function:** Đặt một cái tên cụ thể cho hàm (ví dụ: `functionName`).
* **Parentheses that tells it is a function:** Cặp ngoặc đơn ngay sau tên hàm `()` biểu thị đây là một hàm thực thi.
* **Open the function with a curly brace:** Mở đầu khối lệnh logic của hàm bằng dấu ngoặc nhọn `{`.
* **Code to be executed:** Nội dung các đoạn mã logic cần thực thi nằm bên trong thân hàm.
* **End the function with a closing curly brace:** Kết thúc và đóng phạm vi hàm bằng dấu ngoặc nhọn đóng `}`.
...
2. Đăng ký hàm (`registerFn`): Hàm sẽ được khai báo ở phạm vi global scope nên cần đảm bảo tên hàm (name) là duy nhất.
   ```javascript
   BSS_B2B.support.customFn.registerFn(function doSomething() {
       console.log("do something");
   })
   ```
3. Thêm hàm thực thi ngay (`addRunnableFn`): Hàm này sẽ tự động chạy ngay khi toolkit bắt đầu khởi tạo.
   ```javascript
   BSS_B2B.support.customFn.addRunnableFn(function process() {
       console.log("hello world");
       doSomething();
       
       BSS_B2B.addFilter('custom:product_page/cp_pl_show_price', (priceHtml, priceElement) => {
           if (BSS_B2B.page.isProductPage()) {
               const el = document.querySelector('.sticky-add-to-cart [ref="priceContainer"]');
               if (el) setTimeout(() => el.innerHTML = priceElement.innerHTML, 100);
               return priceHtml;
           }
       });
   })
...
Mọi ý kiến đóng góp phát triển công cụ, báo cáo phát hiện lỗi (bug report) hoặc đề xuất cải tiến tính năng (feature improvements), vui lòng thực hiện thông qua các kênh sau:
...
2. **Pull Requests:** Nếu bạn đã có sẵn mã nguồn khắc phục hoặc cải tiến, hãy mở PR thẳng vào nhánh `main` để đội ngũ core dev review và merge.
3. **Internal Contact:** Đối với các vấn đề nội bộ khẩn cấp trong quá trình support khách hàng của BSS, có thể liên hệ trực tiếp qua hệ thống chat nội bộ của team dev dự án.

---

## License
Internal Support Tools — Developed and Maintained by **TripleVi** for BSS Solution.
